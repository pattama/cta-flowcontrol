...
class Context extends EventEmitter {
  ...
  publish() {
    ...
    this.on('accept', function onContextAccept(who) {
      console.log(`${that.from}: ${who} accepted`);
    })
    .on('reject', function onContextReject(who, reject) {
      console.error(`${that.from}: ${who} rejected with`, reject);
    })
    .on('done', function onContextDone(who, res) {
      console.log(`${that.from}: ${who} done with ${JSON.stringify(res)}`);
    })
    .on('error', function onContextError(who, err) {
      console.error(`${that.from}: ${who} failed with`, err);
    });
