...
class Context extends EventEmitter {
  ...
  publish() {
    ...
    .on('done', function onContextDone(who, res) {
      that.logger.info(`${that.from}: ${who} done with`, res);
    })
  ...
  /**
   * Emit an event with data - wrap native emit to inject the brick name
   * In your brick process method you can call context.send('done', 'ok')
   * instead of context.emit('done', 'brickname', 'ok')
   * @param event
   * @param data
   */
  send(event, data) {
    const brick = this.from;
    // this.logger.debug({event: event, brick: brick, data: data});
    this.emit(event, brick, data);
  }
}
