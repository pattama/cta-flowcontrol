...
class Cement {
  ...
  constructor(configuration) {
    ...
    // this.logger = logger(configuration.logger, 'Cement');

    this.validate(configuration);

    // Instantiating Cement tools
    // TODO Support singleton property
    // TODO Convert dependencies to object with custom references
    this.tools = {};
    const globalTools = [];
    if (Array.isArray(configuration.tools)) {
      configuration.tools.forEach((tool) => {
        const Tool = require(tool.module);
        const toolDependencies = {};
        if (Array.isArray(tool.dependencies)) {
          tool.dependencies.forEach((dependency) => {
            toolDependencies[dependency] = that.tools[dependency];
          });
        }
        globalTools.forEach((gTool) => {
          if (gTool !== tool.name) {
            toolDependencies[gTool] = that.tools[gTool];
          }
        });
        that.tools[tool.name] = new Tool(tool.properties, toolDependencies);
        if (tool.global === true) {
          globalTools.push(tool.name);
        }
      });
    }
    ...
    // TODO throw if not?
    // TODO Manage author property
    if (!this.tools.logger) {
      const Logger = require('cta-logger');
      this.logger = new Logger();
    } else {
      this.logger = this.tools.logger;
    }

    /* if (!this.modules.messaging) {
      const Messaging = require('cta-messaging');
      this.modules.messaging = new Messaging({logger: this.modules.logger});
    }
    if (!this.modules.healthCheck) {
      const HealthCheck = require('cta-healthcheck');
      this.modules.healthCheck = new HealthCheck({}, {logger: this.modules.logger, messaging: this.modules.messaging});
    }*/
    ...
    configuration.bricks.forEach((brickConfig, brickIndex) => {
      ...
      if (Array.isArray(brickConfig.dependencies)) {
        brickConfig.dependencies.forEach((e) => {
          brickDependencies[e] = that.tools[e];
        });
      }
      globalTools.forEach((gTool) => {
        brickDependencies[gTool] = that.tools[gTool];
      });
      try {
  ...
  /**
   * register/update brick health
   * @param brick
   * @param status
   */
  /* health(brick, status) {
    this.logger.info(`Health updated for brick ${brick}`, status);
    this.modules.healthCheck.update(brick, status);
  } */
}
