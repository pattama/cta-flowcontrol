...
class Cement extends SmartEventEmitter {
  ...
  constructor(configuration, dirname) {
    ...
    configuration.bricks.forEach((brickConfig, brickIndex) => {
      ...
      try {
        cementHelper = new CementHelper(this, brickConfig.name, brickDependencies, configuration.properties);
      } catch (error) {
    ...
    Promise.all(initialized)
      ...
      .catch((err) => {
        const errMsg = `Can't initialize all bricks: ${err.message}.`;
        that.logger.error(errMsg, err);
