...
class Cement {
  ...
  constructor(configuration) {
    ...
    // Instantiating Cement modules
    this.modules = {};
    if (Array.isArray(configuration.modules)) {
      configuration.modules.forEach((module) => {
        const Module = require(module.module);
        const dependencies = {};
        if (Array.isArray(module.dependencies)) {
          module.dependencies.forEach((dependency) => {
            dependencies[dependency] = that.modules[dependency];
          });
        }
        that.modules[module.name] = new Module(module.properties, dependencies);
      });
    }
    ...
    configuration.bricks.forEach((brickConfig, brickIndex) => {
      ...
      if (brickConfig.dependencies) {
        brickConfig.dependencies.forEach((e) => {
          dependencies[e] = that.modules[e];
        });
      }
      try {
        cementHelper = new CementHelper(this, brickConfig.name, dependencies);
      } catch (error) {
