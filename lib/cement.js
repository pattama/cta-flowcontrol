...
class Cement {
  ...
  constructor(configuration) {
    ...
    that.validate(configuration);

    // this.modules = modules(configuration.modules);
    that.modules = {};

    if (Array.isArray(configuration.modules)) {
      configuration.modules.forEach((e) => {
        const Module = require(e.module);
        that.modules[e.name] = new Module(e.properties);
      });
    }
