...
class Cement extends EventEmitter {
  ...
  constructor(configuration) {
    ...
    this.bootstraped = false; // set to true when all bricks are initialized & cement bootstraped
    ...
    configuration.bricks.forEach((brickConfig, brickIndex) => {
      ...
      BrickInstance.on('initialized', function() {
        ...
        if (that.initializedBricks === configuration.bricks.length) {
          ...
          that.bootstrap();
          that.bootstraped = true;
          that.emit('bootstraped');
          that.start();
        }
      });
   ...
   * Should be called after the cement is bootstraped
   * */
  start() {
    const that = this;
  ...
  // make sure to call "publish" method only when the cement is bootstraped
  // for test case "CementHelper - publish Context" to succeed

  publish(context) {
    const that = this;
    if (that.bootstraped === true) {
      that._publish(context);
    } else {
      that.on('bootstraped', function() {
        that._publish(context);
      });
    }
  }
