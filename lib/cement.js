  ...
  constructor(configuration) {
    ...
    if (Array.isArray(configuration.tools)) {
      configuration.tools.forEach((tool) => {
        let Tool;
        try {
          Tool = require(tool.module);
        } catch (error) {
          throw (new Error(`failed to load module '${tool.module}': ${error}`));
        }
        const toolDependencies = {};
        configuration.tools.filter((gTool) => {
          return (gTool.name !== tool.name) && (gTool.scope === 'all' || gTool.scope === 'tools');
        }).forEach((gTool) => {
          toolDependencies[gTool.name] = that.tools[gTool];
        });
    ...
    configuration.bricks.forEach((brickConfig, brickIndex) => {
      ...
      if (Array.isArray(configuration.tools)) {
        configuration.tools.filter((gTool) => {
          ...
          brickDependencies[gTool.name] = that.tools[gTool];
        });
      }
