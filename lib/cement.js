...
class Cement extends EventEmitter {
   ...
   * @constructs Cement
   * @param {CementConfig|String} conf - configuration object or full path to a config directory for instantiating a new Cement
   */
  constructor(conf) {
    super();
    const that = this;

    const configuration = (typeof conf === 'string') ? config(conf) : conf;

    // first, instantiate cement logger in case of dirty outputs
    const loggerConf = (configuration && typeof configuration === 'object' && 'logger' in configuration) ? configuration.logger : {};
    that.logger = logger(loggerConf, 'Cement');
    ...
    configuration.bricks.forEach((brickConfig, brickIndex) => {
      ...
      let brickLogger;
      try {
        brickLogger = logger(brickConfig.logger || configuration.logger, brickConfig.name);
        cementHelper = new CementHelper(this, brickConfig.name, brickLogger);
      } catch (error) {
      ...
      BrickInstance.on('initialized', function() {
        that.logger.info(`Brick ${brickConfig.name} initialized`);
        that.initializedBricks++;
        if (that.initializedBricks === configuration.bricks.length) {
          that.logger.info('All Bricks are initialized');
          that.bootstrap();
