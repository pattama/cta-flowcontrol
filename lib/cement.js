...
class Cement {
    ...
    configuration.bricks.forEach((brickConfig, brickIndex) => {
      ...
      if (Array.isArray(brickConfig.publish)) {
        brickConfig.publish.forEach((pubContract) => {
          ...
          this.channels.get(pubContract.topic).addPublisher(brickConfig.name, pubContract.data, BrickInstance);
        });
      }
      ...
      if (Array.isArray(brickConfig.subscribe)) {
        brickConfig.subscribe.forEach((subContract) => {
          ...
          this.channels.get(subContract.topic).addSubscriber(brickConfig.name, subContract.data, BrickInstance);
        });
      }
    });
  }
   ...
   */
  publish(context) {
    const destinations = this.getDestinations(context.from, context.data);
