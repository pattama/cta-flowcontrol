              ...
              if (!Array.isArray(link.jobs)) {
                ...
                link.jobs.forEach((job, jobIndex) => {
                  if ('type' in job && typeof job.type !== 'string') {
                    throw (new Error(`incorrect 'type' string property in bricks[${brickIndex}].links[${linkIndex}].jobs[${jobIndex}]`));
                  }

                  if ('quality' in job && typeof job.quality !== 'string') {
                    throw (new Error(`incorrect 'quality' string property in bricks[${brickIndex}].links[${linkIndex}].jobs[${jobIndex}]`));
                  }

                  if ('except' in job && typeof job.except !== 'boolean') {
                    throw (new Error(`incorrect 'except' boolean property in bricks[${brickIndex}].links[${linkIndex}].jobs[${jobIndex}]`));
                  }
                });
              }
            }
  ...
  canSend(context, destination) {
    ...
    if ('links' in source.configuration) {
      ...
      if (typeof link !== 'undefined') {
        if (!('jobs' in link) || link.jobs.length === 0) {
          canSend = true;
        } else {
          canSend = link.jobs.some((jobdefinition) => {
            let matched = true;
            matched &= jobdefinition.quality === undefined || jobdefinition.quality === context.data.nature.quality;
            matched &= jobdefinition.type === undefined || jobdefinition.type === context.data.nature.type;
            if ('except' in jobdefinition && jobdefinition.except === true) {
              return !matched;
            }
            return matched;
          });
